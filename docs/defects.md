...
| DEF-11 | 2026-09-09 | `prompts/persona.md` | Asked for the TRM of 30 and 31 December 2025, answered $4,420.00 and $4,409.15 — the real December 2024 close, not the $3,706.97 and $3,757.08 of the date asked — heading both replies "Confirmado con fuentes" | The journal shows no `tools:` line for either turn: it never searched, and both replies were headed as confirmed with sources anyway. Not invention, which is what this row said first: the figures were real — the December 2024 close — given for December 2025. What set it off is not established. The same question asked with an empty history and no facts searched 5/5 and answered correctly (probed 2026-09-11), so the bare question does not reproduce it, and the state of the window that day is the working hypothesis, not an observation. Same class as DEF-10 and same day, but no address is involved, so the fence built for DEF-10 cannot see it | open | reported | claims-what-it-cannot-check | open |
| DEF-12 | 2026-09-15 | `core/memory_manager.py` | A merge whose replacement is the wording of one of the facts it retires leaves nothing active: the insert collides with the unique index, the error is swallowed as a duplicate, and the retire runs anyway | DEF-6's fence sits in `analyze_memory` and only sees a pass with no new facts; here `new_facts` is not empty, so the retire reaches `apply_memory_changes` unchecked | A fact is never retired by its own wording, and a pass that saved nothing new retires nothing — DEF-6's rule again, applied where what was saved is known; both fences log at warning — `fe2da6d`, second rule from its review | review — seven of eight candidate reviewer models named it while being measured on the commit that introduced item 35c | fence-sees-one-shape | fixed |
| DEF-13 | 2026-09-15 | `tests/test_conversation.py` | Three tests that script the model's replies in sequence stayed green after item 35j slice 1 while the behaviour they exist for no longer ran: the new classifier call consumed the reply carrying the tool call, the next reply became the answer, and each assertion held for another reason | Nine sequenced tests go through `handle_turn`; the verdict was pinned only on the six that went red, and the three whose assertions happened to survive were never audited | `STABLE_VERDICT` on all nine, found by listing every sequenced test that reaches `handle_turn` rather than by running the suite | review — all three reviewer models named it on the first run of `method/tools/review.py` | measured-only-where-it-succeeds | fixed |
...
| measured-only-where-it-succeeds | A change was verified by counting what it did right, by an instrument blind to what it destroyed | DEF-6, DEF-13 |
