...
| 3 | **El despliegue** — usuario del sistema, `/opt/tabris`, rama de producción con su primera etiqueta, clon, ambiente propio y archivo de llaves; la base la crea el servicio vacía en su primer arranque | AC4, AC13, AC14, AC15, AC16, D2, D3, D4 | La suite completa corre verde dentro del despliegue, como ese usuario; el primer arranque crea la carpeta de datos y la base con permisos de solo-dueño, y no reconoce a nadie | ✅ |
...
- **El archivo de llaves tiene que ser `CLAVE=valor`, sin espacios ni comillas.** El administrador de servicios no aplica las reglas de la librería de Python: un valor entrecomillado llega con las comillas pegadas y falla como si la llave estuviera vencida. La rebanada 7 lo verifica junto con las variables que declara `.env.example` (D8).
- **`setup.sh` recrea un `.env` de plantilla dentro del clon.** Con las variables declaradas pero vacías, un arranque sin llaves falla de forma confusa en vez de ruidosa. La rebanada 7 define si el guion de despliegue lo borra o no lo usa.
