...
|---|---|---|---|---|
| 1 | The verdict and the forced first round. The classification prompt moves from `tools/probe_freshness.py` into `core/`, and the probe imports it from there, so the measured prompt is the deployed one. `handle_turn` asks the verdict through the `router` role after the role is known — text turns and image turns alike, never on `exit` — and logs `freshness: fresh` / `stable` / `no verdict`. On `fresh` (or no verdict) `run_with_tools` sends the first round with `tool_choice` naming `web_search`; `providers.chat` passes the parameter through. On `stable` nothing changes. `CONTRIBUTING.md` gains the probe rule and `CHANGELOG.md` its line, in the same change. | AC1 (forcing), AC3, AC4, AC6, AC7, AC8 | Live on Discord with yesterday's report still in the window: a question about today's rate shows `freshness: fresh` followed by `tools: … web_search` in the journal and a reply with today's value and date; a translation shows `freshness: stable` and no `tools:` line; the same question sent as a voice note and beside a photo take the same path. Tests: the prompt reaches the model with the message wrapped as data; a verdict other than the two words is `no verdict`; `tool_choice` is present on the first round on `fresh` and absent on `stable` and on later rounds. | ✅ 2026-09-15, `v0.1.19`; verified live with today's report in the window, by text, voice note and photo |
| 2 | The net beneath the forcing. A `fresh` turn that brought no outside content — no `web_search` executed and no page read — goes back to the model twice, in the shape of the link correction: first to search, then to write the answer again without the value nothing backs, so what was verified still reaches the user (spec AC2). Only an answer returned word for word is withheld, and the notice then names what the turn applied, from what the code itself executed. A search that ran and brought nothing, or a successful `web_fetch`, is not the net's case: the reply goes through, stating the absence is the model's (spec failure rows "chain failed entirely" and "no result is the true answer", open question 2), and the journal counts the turn once. Journal lines `freshness: forced search missing, corrected`, `… answer without the value requested`, `… withheld` and `freshness: required search brought nothing`. | AC1 (net), AC2 (chain failure) | Tests only, with the model double ignoring the forcing: correction, then rewrite, then the notice naming a fact the turn saved; the reply passing when a page was read, and withheld when the reading failed; the count line once per turn when the search ran empty. It cannot be provoked live on demand — the three `general` models obeyed 3/3 under the defect's condition — so it stands on tests and on the correction cycle 35i already runs in production. Declared as such here rather than found out later. | ⬜ |
| 3 | Close. `docs/defects.md` DEF-11 corrected — the figures were real values of another year and the trigger is the window's state, not invention; the third live case is already in `framing.md`. `PLAN.md` item marked, the next patch tag cut (`v0.1.19` went out at slice 1, because the live verification needs code in service). The two persona rules stay until the journal of two weeks in service says whether the first can go. | — | The tag in service, the journal of the first days read for `freshness:` lines, every real miss added to the probe's lot. | ⬜ |
...
- Review of slice 2 (2026-09-21): `language` keeps its default in `run_with_tools` on purpose. A caller that forgets it costs one notice in the wrong language; a required parameter would cost the turn. The owner ranks one message above one error.
