...
| 5 | **Retire the old deployment**, seven days after slice 3 — units, `/opt/tabris`, the system user, `/var/backups/tabris` | M3 | Nothing of it is left on the old host, and Tabris is still answering from the new one | ✅ 2026-09-24 (four days past the window; units confirmed disabled and unloaded, then removed; `/opt/tabris`, `tabris` and `/var/backups/tabris` gone — `id tabris` and `ls -d` on both paths returned not-found) |
