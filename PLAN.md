...
34e. ⬜ Account linking via link-code — split out of item 34 on 2026-07-30 (design unchanged, see item 34's header; `user_channels` already supports it with zero migration). Today the CLI and Discord are two separate users with separate memories: Rumpel's CLI profile holds his real memory (7 facts, 116 messages, `America/Bogota`) while his Discord profile is near-empty and stuck on `UTC`, so Discord reports the wrong local time. Real work beyond the code: a new table for short-lived codes, a re-point (not insert) path for an existing `(channel, key)` row, tools for request/redeem mirroring `forget_fact`'s security boundary (model supplies only the code), and one genuine design decision — what happens to the throwaway profile auto-created on the new channel before linking (discard vs migrate its facts/messages). Rumpel's call 2026-07-30: build it properly rather than take the one-line SQL shortcut for his own account, but not now. Truly needed by 34d (Telegram), where a third channel makes the manual shortcut untenable.
   ...
   - ⬜ **Redundant prompt construction in the adapters (found 2026-08-11).** `main.py` and `discord_ch.py` each build a system prompt that `handle_turn` overwrites on the first turn, so its content never reaches a model — left over from when the prompt was built once per session. Deleting it shifts `last_analyzed_index`, which counts the system message, so the watermark has to move with it or distillation re-reads one analysed message.
   - **Remaining:** a live run of the profile correction (persona wording is not testable), the redundant prompt construction in the adapters, and the non-atomic `create_user` + `register_user_channel` pair.
...
34g. ⬜ **A reply too long for Discord is silently lost** (found live 2026-08-11). Discord rejects any message over 2000 characters; the adapter sends without splitting, and the reply is already saved, so the user sees silence while the model's history keeps an answer they never read — which then makes it refer back to things the user never saw. Three parts: split the reply in the adapter, tell the user when a send fails instead of failing silently, and retire the undelivered reply using the unused `messages.is_active` column so both sides hold the same conversation.
