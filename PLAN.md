...
34c. 🔶 Data privacy minimums (code review 2026-07-02) — gate before onboarding beta-testers (§6). Run through the `/method` cycle, documented in `docs/34c/`. Split into three parts; the first two are settled, the third is open.
   ...
   - ⬜ **Part 3 — automatic retention of old messages.** Delete conversation history older than N months, on its own cycle. Not started; the only part of the original scope still open.
