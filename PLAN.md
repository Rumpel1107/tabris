...
34e. ⬜ Account linking via link-code — split out of item 34 on 2026-07-30 (design unchanged, see item 34's header; `user_channels` already supports it with zero migration). Today the CLI and Discord are two separate users with separate memories: Rumpel's CLI profile holds his real memory (7 facts, 116 messages, `America/Bogota`) while his Discord profile is near-empty and stuck on `UTC`, so Discord reports the wrong local time. Real work beyond the code: a new table for short-lived codes, a re-point (not insert) path for an existing `(channel, key)` row, tools for request/redeem mirroring `forget_fact`'s security boundary (model supplies only the code), and one genuine design decision — what happens to the throwaway profile auto-created on the new channel before linking (discard vs migrate its facts/messages). Rumpel's call 2026-07-30: build it properly rather than take the one-line SQL shortcut for his own account, but not now. Truly needed by 34d (Telegram), where a third channel makes the manual shortcut untenable.
   ...
   - ⬜ **Pre-existing gap, NOT introduced by this item:** `create_user` and `register_user_channel` run in separate transactions (`onboard_user` in `main.py`, `handle_message` in `discord_ch.py`), so a crash between them leaves a user row that no channel points to — unreachable. Microsecond window, orphan row only, no data loss or security impact. Fix = both writes in one transaction. Open decision: fold into the wiring or leave. `redeem_link_code` is already atomic.
   ...
   - 🔄 (2026-08-07) **State-driven onboarding started:** `advance_onboarding(session, user_input)` in `core/onboarding.py` consumes one message, moves `session.onboarding_step` and returns what to say — no printing, no input, no DB yet. Steps done: first contact (detect language → ask to confirm), `language` (confirmed → greet and ask for a link code or the name; rejected → ask which language), `language_ask` (detect from free text, then the same greeting). Remaining steps: `link_or_name`, city, ambiguity re-ask, read-back, user creation. `db_path` and the fields holding the pending name/city get added when the step that needs them arrives, not before.
   ...
   - ⬜ **Profile visibility (agreed 2026-08-07):** the linked channels join name/city/language in the per-turn prompt block, by channel name only, never keys ("we talk over CLI and Discord"). Lets a person audit their own record and spot a link they do not recognize, and doubles as the confirmation that a redeem worked. Chosen over an on-demand tool: channels change rarely and a line costs no model call.
   - ⬜ **Profile correction (agreed 2026-08-07, not designed):** a way to fix name/city/language after onboarding. Today a wrong profile is permanent — `users` rows are never updated except `update_user_language`, which nothing calls.
   - **Remaining:** entry-flow fork + shared state-driven onboarding + request tool (`forget_fact`-style: the model supplies nothing, the session binds `user_id`) + redeem at adapter level, pre-session + `strings.py` keys (es/en) + `persona.md` mention + profile visibility + profile correction.
