...
33d. ⬜ Onboarding friendliness (found 2026-07-10, real run). `resolve_language` (`main.py`) exact-matches the confirmation string — natural replies ("si, por favor", "sí") fail and silently cascade to an English default. Fixes: tolerant matching (normalize + match first word) in `resolve_language` and the es/en fallback; confirm extracted name back to the user; bilingual `ask_name` prompt.
33e. ⬜ Timezone + real-time grounding gaps (found 2026-07-16, real run, after wiring `web_fetch` + user name into the system prompt). Two issues seen in the same session: (1) `build_system_prompt`'s `## Current context` block always uses server-local time (`datetime.now()`, server runs UTC) — there is no timezone handling anywhere in the code, so the model tries to mentally offset to the user's real timezone and gives inconsistent, self-contradicting dates across turns, even after "user is in Bogota" was saved as a fact. (2) For time-sensitive queries (e.g. "today's" match results) the model appears to fabricate specific answers instead of calling `web_search`, despite `persona.md` instructing it to — evidence: different, unverified scores tied to different (also wrong) "today" dates within the same session, not confirmed via a real tool call. Needs design before fixing: where timezone lives (new per-user fact vs a `users` column) and how/where `now` gets converted before formatting; and whether `persona.md` needs stronger, less optional language forcing `web_search` for time-sensitive claims.
