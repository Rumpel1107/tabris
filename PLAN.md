...
33f. ⬜ Normalize stored location (found 2026-07-17, real run). `onboard_user` stores the raw city answer verbatim (e.g. "Claro, vivo en Madrid Cundinamarca en Colombia"), so the prompt reads "located in <sentence>". Clean it to a city name (LLM helper, twin of `extract_name`) before `create_user`. Timezone is unaffected (already handled by `resolve_timezone`).
