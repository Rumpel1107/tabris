...
- **Ask, then apply.** Ask for authorization before each change, then make it yourself with the editor tools, so it lands as a diff the user reviews in the editor. Do not hand over code blocks to paste by hand. One edit per coherent block, sized so the diff stays readable — group many similar lines into a couple of block edits rather than one sweeping rewrite or twenty micro-edits. Never write a project file from the terminal (no `sed -i`, no heredoc rewrite, no redirect), even when scripting would match more exactly.
